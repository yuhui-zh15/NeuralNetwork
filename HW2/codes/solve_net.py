...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq):
    ...
    for input, label in data_iterator(inputs, labels, batch_size):
        ...
        if iter_counter % disp_freq == 0:
            # loss_list_train.append(np.mean(loss_list))
            # acc_list_train.append(np.mean(acc_list))
...
def test_net(model, loss, inputs, labels, batch_size):
    ...
    # loss_list_test.append(np.mean(loss_list))
    # acc_list_test.append(np.mean(acc_list))
    # json.dump(loss_list_test, open('loss.test.json', 'w'))
    # json.dump(acc_list_test, open('acc.test.json', 'w'))
    # json.dump(loss_list_train, open('loss.train.json', 'w'))
    # json.dump(acc_list_train, open('acc.train.json', 'w'))
